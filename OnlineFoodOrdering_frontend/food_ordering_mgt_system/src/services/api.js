...
export const restaurantApi = {
  ...
  getAllPaginated: (page = 0, size = 10) =>
    api.get(`/restaurants/paginated?page=${page}&size=${size}`),
  ...
  getByLocation: (locationId) => api.get(`/restaurants/location/${locationId}`),
  getByProvince: (provinceId) => api.get(`/restaurants/province/${provinceId}`),
  search: (term) => api.get(`/restaurants/search/${term}`),
};
...
export const menuItemApi = {
  ...
  getByPriceRange: (min, max) =>
    api.get(`/menu-items/price-range?min=${min}&max=${max}`),
  search: (name) => api.get(`/menu-items/search?name=${name}`),
};
...
// ========== Auth API (to be implemented on backend) ==========
